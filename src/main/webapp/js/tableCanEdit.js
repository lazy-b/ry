...
function SetTableCanEdit(table){
	//设置除最后一个表头单元格的属性：EditType="textBox"
	var oTh=table.getElementsByTagName("th");
	for(var i=0;i<oTh.length-1;i++){
		oTh[i].setAttribute("EditType","textBox");
	}
...
for(var i=table.rows.length-1;i>0;i--){
   ...
   var chkOrder = cells[cells.length-1].firstChild;
   if(chkOrder){
...
for(var i=1; i<table.rows.length;i++){
	/*editTable传入的表格对象*/
   tableData.push(GetRowData(editTable.rows[i]));
}
return tableData;
}
//提取指定行的数据，JSON格式
